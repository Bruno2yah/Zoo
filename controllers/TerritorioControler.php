...
class UserController{
    ...
    public function store(){
        ...
        $camposObrigatorios = ['nome', 'email', 'telefone','senha'];
        foreach ($camposObrigatorios as $campo) {
            if (!isset($_POST[$campo]) || empty($_POST[$campo])) {
                // Se algum campo estiver faltando ou vazio, retorna uma resposta de erro
                http_response_code(402);
                $veri = ['mensagem' => 'Todos os campos são obrigatórios'];
    ...
    public function checkCredentials(){
      // Verifica se todos os campos necessários foram enviados
        ...
        try {
            // Atualiza os dados do usuário no banco de dados
            $dados = UserDao::checkCredentials($user['email'],$user['senha']);
            if($dados){
                $userId = [
                    'email' => $_POST['email'],
                    'senha' => $_POST['senha'],
           ...
           echo json_encode($userId);
            ...
            // Se ocorrer um erro durante a atualização, retorna uma resposta de erro com a mensagem de exceção
            http_response_code(500); // 500 Internal Server Error
            echo json_encode(['mensagem' => 'Erro nas informações do usuário: ' . $e->getMessage()]);
        }
    }
